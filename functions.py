...
def RotatingCoilAnalysisTurn(Fabs, Fcmp, knAbs, knCmp, MagOrder, Rref, AnalysisOptions,Phi0):
    #fft
    ...
    Phitonorm=SignalPhase/MagOrder#This is to have the correct signs on B_Main_Rot for each magnet current when normalizing to the main field
    ...
    B_Main_Rotated_norm = np.real((np.exp(-1j*(Phitonorm)*MagOrder) * (c_sens_abs[MagOrder-1])))
    B_Main_Rotated_skew = np.imag((np.exp(-1j*(Phitonorm)*MagOrder) * (c_sens_abs[MagOrder-1])))
    B_Main=np.absolute(c_sens_abs[MagOrder-1])
...
def ContinuousRotatingCoilAnalysis (datafile, p_turn, knAbs, knCmp, MagOrder, Rref, AnalysisOptions,step,Phi0):
    ...
    for turn in turnlst:
        ...
        [Norm_cmp,
         ...
         x,
         y,
         PhiOut]=RotatingCoilAnalysisTurn(Fabs, Fcmp, knAbs, knCmp, MagOrder, Rref, AnalysisOptions,Phi0)
...
def ReadRoxie(file,n=15,NS="NS",bothDipoles=False):

    # file = filedialog.askopenfilename()
    # file_name=file.split('\\')[0:-1]
    # file_folder = '\\'.join(file.split('\\')[0:-1])
    # file="C:\Maq Virtual\Shared_Folder\MCBXFA_BOTH_RAMPING_NOCRYO_NEW_CURRENT_AND_SPACERS_Luis.output"
    # file="C:\Maq Virtual\Shared_Folder\Head-2mm.output"
    ...
    for i in range(2,(len(a)-1),1):
        ...
        # if i==len(a):
        #     b=b.split('NUMBER OF OBJECTIVES AND CONSTRAINTS')[0]
        # else:
        #     pass
        ...
        if NS == "NS":
            ...
            if c<n:
                ...
                alles['B'+str(c+1)]=new['Signal'].fillna(method='ffill')
                
            else:
                alles['A'+str(c-(n-1))]=new['Signal'].fillna(method='ffill')
            ...
            alles['Position']=new['Position']
            alles['A'+str(c+1)]=new['Signal'].fillna(method='ffill')
    ...
    shift=abs(alles['Position'].min()+alles["Position"].max())/2
    ...
    if bothDipoles==False:
        ...
        # MP=10000*(alles.iloc[int(max(alles.index)/2)]/(max(alles.iloc[int(max(alles.index)/2)])))
        # MP=MP.drop(['Position'],axis=0)
        # orient=MP[MP==max(MP)].index[0][0] 
        # tempind=[]
        # for item in MP.index:
        #     tempind.append(orient in item)
        # MP=MP[tempind]
            
    
    alles.to_excel(file+'\\'+"Roxie_Multipoles.xlsx")
    
    return [MP,alles]

def interpolate(file,df,step=0.5):
    rango=pd.DataFrame(np.arange(int(df['Position'].min()),int(df['Position'].max()),step),columns=['Position'])
    print(rango)
    result=df.merge(rango,how="outer").sort_values(by='Position').interpolate(method='linear')
    result=result.merge(rango,how="right")
    result.to_excel(file+"Multipoles_interpolated_"+str(step)+"mm.xlsx")
    return result
...
def integrate(df,start,end,step):
    if (start-end)/step != 0.0:
        print('Integrated region must be a multipleLG. In the lab of step')
        return None